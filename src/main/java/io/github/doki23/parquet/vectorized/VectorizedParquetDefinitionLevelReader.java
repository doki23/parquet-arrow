...
public final class VectorizedParquetDefinitionLevelReader extends BaseVectorizedParquetValuesReader {
  ...
  private void setNextNValuesInVector(int typeWidth, NullabilityHolder nullabilityHolder,
                                      ValuesAsBytesReader valuesReader, int bufferIdx, FieldVector vector,
                                      int numValues) {
  ...
  abstract class NumericBaseReader {
    public void nextBatch(final FieldVector vector, final int startOffset, final int typeWidth, final int numValsToRead,
                          NullabilityHolder nullabilityHolder, ValuesAsBytesReader valuesReader) {
    ...
    public void nextDictEncodedBatch(final FieldVector vector, final int startOffset, final int typeWidth,
      ...
      while (left > 0) {
        ...
        switch (mode) {
          case RLE:
            if (currentValue == maxDefLevel) {
              nextDictEncodedVal(vector, idx, dictionaryEncodedValuesReader, dict, mode, numValues, nullabilityHolder,
                  typeWidth);
            } else {
    ...
    protected abstract void nextVal(FieldVector vector, int idx, ValuesAsBytesReader valuesReader, Mode mode);

    protected abstract void nextDictEncodedVal(FieldVector vector, int idx,
                                               VectorizedDictionaryEncodedParquetValuesReader dictionaryEncodedValuesReader,
                                               Dictionary dict, Mode mode, int numValues, NullabilityHolder holder,
                                               int typeWidth);
  }
  ...
  class LongReader extends NumericBaseReader {
    ...
    @Override
    protected void nextDictEncodedVal(FieldVector vector, int idx,
                                      VectorizedDictionaryEncodedParquetValuesReader dictionaryEncodedValuesReader,
                                      Dictionary dict, Mode mode, int numValues, NullabilityHolder holder,
                                      int typeWidth) {
  ...
  class DoubleReader extends NumericBaseReader {
    ...
    protected void nextDictEncodedVal(FieldVector vector, int idx,
                                      ...
                                      int typeWidth) {
      if (Mode.RLE.equals(mode)) {
        dictionaryEncodedValuesReader.doubleDictEncodedReader()
            .nextBatch(vector, idx, numValues, dict, holder, typeWidth);
      } else if (Mode.PACKED.equals(mode)) {
        vector.getDataBuffer()
  ...
  class FloatReader extends NumericBaseReader {
    ...
    protected void nextDictEncodedVal(FieldVector vector, int idx,
                                      ...
                                      int typeWidth) {
      if (Mode.RLE.equals(mode)) {
        dictionaryEncodedValuesReader.floatDictEncodedReader()
            .nextBatch(vector, idx, numValues, dict, holder, typeWidth);
      } else if (Mode.PACKED.equals(mode)) {
        vector.getDataBuffer()
  ...
  class IntegerReader extends NumericBaseReader {
    ...
    protected void nextDictEncodedVal(FieldVector vector, int idx,
                                      ...
                                      int typeWidth) {
      if (Mode.RLE.equals(mode)) {
        dictionaryEncodedValuesReader.integerDictEncodedReader()
            .nextBatch(vector, idx, numValues, dict, holder, typeWidth);
      } else if (Mode.PACKED.equals(mode)) {
        vector.getDataBuffer()
  ...
  abstract class BaseReader {
    ...
    public void nextDictEncodedBatch(final FieldVector vector, final int startOffset, final int typeWidth,
      ...
      while (left > 0) {
        ...
        switch (mode) {
          case RLE:
            if (currentValue == maxDefLevel) {
              nextDictEncodedVal(vector, idx, dictionaryEncodedValuesReader, numValues, dict, nullabilityHolder,
                  typeWidth, mode);
            } else {
              setNulls(nullabilityHolder, idx, numValues, validityBuffer);
            }
    ...
    protected abstract void nextVal(FieldVector vector, int idx, ValuesAsBytesReader valuesReader, int typeWidth,
                                    byte[] byteArray);

    protected abstract void nextDictEncodedVal(FieldVector vector, int idx,
                                               VectorizedDictionaryEncodedParquetValuesReader reader,
                                               int numValuesToRead, Dictionary dict,
                                               NullabilityHolder nullabilityHolder, int typeWidth, Mode mode);
  }
  ...
  class TimestampMillisReader extends BaseReader {
    ...
    @Override
    protected void nextDictEncodedVal(FieldVector vector, int idx,
                                      VectorizedDictionaryEncodedParquetValuesReader reader, int numValuesToRead,
                                      Dictionary dict, NullabilityHolder nullabilityHolder, int typeWidth, Mode mode) {
      if (Mode.RLE.equals(mode)) {
  ...
  class TimestampInt96Reader extends BaseReader {
    ...
    protected void nextDictEncodedVal(FieldVector vector, int idx,
      ...
      switch (mode) {
        case RLE:
          reader.timestampInt96DictEncodedReader()
              .nextBatch(vector, idx, numValuesToRead, dict, nullabilityHolder, typeWidth);
          break;
        case PACKED:
  ...
  class FixedSizeBinaryReader extends BaseReader {
    ...
    protected void nextDictEncodedVal(FieldVector vector, int idx,
                                      ...
                                      Dictionary dict, NullabilityHolder nullabilityHolder, int typeWidth, Mode mode) {
      if (Mode.RLE.equals(mode)) {
        reader.fixedSizeBinaryDictEncodedReader()
            .nextBatch(vector, idx, numValuesToRead, dict, nullabilityHolder, typeWidth);
      } else if (Mode.PACKED.equals(mode)) {

...
public abstract class BaseBatchReader<T> implements VectorizedReader<T> {
    ...
    protected BaseBatchReader(List<VectorizedReader<?>> readers) {
        this.readers = readers.stream().map(VectorizedArrowReader.class::cast).toArray(VectorizedArrowReader[]::new);
        this.vectorHolders = new VectorHolder[readers.size()];
    }

    @Override
    public void setRowGroupInfo(PageReadStore pageStore, ColumnChunkMetaData... columnChunkMetaData) {
        Objects.requireNonNull(columnChunkMetaData, "columnChunkMetaData must not be null");
        Preconditions.checkArgument(
                readers.length == columnChunkMetaData.length,
                "length of columnChunkMetaData must match that of columns");
        for (int i = 0; i < readers.length; i++) {
            if (readers[i] != null) {
                readers[i].setRowGroupInfo(pageStore, columnChunkMetaData[i]);
            }
        }
    }
